...
public:
    ...
    void SetPosition(const Effekseer::Handle& handle, const DirectX::XMFLOAT3& position); // 位置設定
    void SetRotation(const Effekseer::Handle& handle, const DirectX::XMFLOAT3& rotation, const float& angle); // 角度設定
    void SetScale(const Effekseer::Handle& handle, const float& scale);                   // 大きさ設定
