...
PSGBufferOut main(GSOut psIn, bool isFrontFace : SV_IsFrontFace)
{
    ...
    if (m.emissiveTexture.index > -1)
    {
        ...
        emissiveColor.rgb = emissive.rgb * emissiveColor_.rgb;
    ...
    float3 T = hasTangent ? normalize(psIn.worldTangent_.xyz) : float3(1, 0, 0);
    float sigma = hasTangent ? psIn.worldTangent_.w : 1.0;
    ...
    // 視線ベクトル
    float3 V = normalize(psIn.worldPosition_.xyz - cameraPosition.xyz);
    ...
    {
        GBufferData data;
        data.baseColor_ = outlineColor_;
        data.emissiveColor_ = outlineColor_.rgb;
        data.worldNormal_ = float3(0, 0, 0);
        data.worldPosition_ = psIn.worldPosition_.xyz;
        data.roughness_ = 0;
        data.metalness_ = 0;
        data.occlusionFactor_ = 0;
        data.occlusionStrength_ = 0;
